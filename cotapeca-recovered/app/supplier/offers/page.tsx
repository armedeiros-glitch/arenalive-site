...
export default function SupplierOffersPage() {
  ...
  useEffect(()=>{
    ...
    void (async()=>{
      ...
      for (const offer of offers) {
        ...
        result.push({
          ...offer,
          vehicle:vehicle?`${vehicle.brand_name} ${vehicle.model} ${vehicle.year}`:'Veículo',
          total:lines.reduce((sum,line)=>sum+line.price_cents,0),
          lines,
        });
      }
  ...
  return <main className="min-h-screen bg-[#f6f2e9] text-[#1b1d22]">
    ...
    <section className="mx-auto max-w-5xl px-5 py-8">
      ...
      {search.get('created')==='1' && <div data-testid="offer-created-success" className="mt-5 rounded-2xl bg-green-50 px-4 py-3 text-sm font-bold text-green-800">Proposta enviada com sucesso.</div>}
      ...
      <div className="mt-7 grid gap-4 md:grid-cols-2">
        {cards.map(card=><article key={card.id} className="rounded-3xl border border-black/10 bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between gap-4">
            <div><h2 className="text-xl font-black">{card.vehicle}</h2><p className="mt-1 text-sm text-black/50">Enviada {relativeTime(card.created_at)}</p></div>
            <span className="rounded-full bg-green-50 px-3 py-1 text-xs font-bold text-green-800">ENVIADA</span>
          </div>
          <div className="mt-5 grid gap-3">
            {card.lines.map((line,index)=><div key={`${line.name}-${index}`} className="rounded-2xl bg-[#f6f2e9] p-4">
              <div className="flex items-start justify-between gap-3"><b>{line.name}</b><b>{money(line.price_cents)}</b></div>
              <p className="mt-1 text-xs text-black/55">{line.brand_name} · {CONDITION_LABELS[line.condition]??line.condition}</p>
            </div>)}
          </div>
