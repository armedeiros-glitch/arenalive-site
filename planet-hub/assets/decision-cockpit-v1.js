(() => {
  ...
  const mount = () => document.querySelector('[data-decision-cockpit]');
  ...
  const shouldSurface = (item, type) => {
    ...
    if (type === 'risk') return !item.followUpDate && (['late', 'today'].includes(bucket) || !item.blockerReason || !item.dependsOn);
  ...
  const executionReason = (item, type) => {
    ...
    if (type === 'follow_up') {
      ...
      return `${dependency}${item.blockerReason ? ` ${item.blockerReason}` : ''} A data definida para cobrar ou revisar chegou.`;
    }
    if (type === 'risk') {
      return [
  ...
  const nextActionFor = (item, type) => {
    if (type === 'follow_up') return `Cobrar ${item.dependsOn || item.responsible || 'a pessoa responsável'} e registrar uma nova previsão.`;
  ...
  const buildAttention = (item) => {
    ...
    return { type, item, score: attentionScore(item, type), reason: executionReason(item, type), nextAction: nextActionFor(item, type) };
  };

  const attentionQueue = (items) => {
    const candidates = items.map(buildAttention).filter((entry) => shouldSurface(entry.item, entry.type)).sort((a, b) => a.score - b.score);
    if (candidates.length) return candidates;
    const fallback = items.filter((item) => !isDeferred(item)).sort((a, b) => focusScore(a) - focusScore(b))[0];
  ...
  const nearestFollowUp = (items) => items.filter((item) => isDeferred(item) && item.followUpDate).sort((a, b) => String(a.followUpDate).localeCompare(String(b.followUpDate)))[0] || null;

  const queueCard = (entry) => {
    const meta = typeMeta(entry.type);
    const due = radar().dueMeta(entry.type === 'follow_up' ? entry.item.followUpDate : entry.item.dueDate);
    return `<button type="button" class="pmh-attention-card tone-${esc(meta.tone)}" data-attention-open="${esc(entry.item.id)}">
      <span class="pmh-attention-kind">${esc(meta.icon)} ${esc(meta.label)}</span><strong>${esc(entry.item.title)}</strong><small>${esc(entry.reason)}</small>
  ...
  const emptyMarkup = (items) => {
    ...
    const nextLabel = next?.followUpDate ? new Intl.DateTimeFormat('pt-BR').format(new Date(`${next.followUpDate}T12:00:00`)) : '';
    return `<div class="pmh-decision-top"><div class="pmh-decision-main"><small>🟢 O QUE PRECISA DA SUA ATENÇÃO AGORA</small><h2>Nada executável exige ação imediata</h2><p>${summary.dependencies ? `${summary.dependencies} item(ns) dependem de informação, aprovação ou retorno externo.` : 'O Radar não encontrou nenhuma pendência ativa.'}</p>${next ? `<div class="pmh-decision-next"><small>PRÓXIMO ACOMPANHAMENTO</small><strong>${esc(next.title)} · ${esc(nextLabel)}</strong></div>` : ''}</div>${next ? `<div class="pmh-decision-actions"><button type="button" data-radar-context="${esc(next.id)}">Revisar dependência</button></div>` : ''}</div>
      <div class="pmh-decision-signals">${signal(summary.executable, ' executáveis')}${signal(summary.dependencies, ' com dependência', summary.dependencies ? 'warning' : '')}${signal(summary.followUps, ' cobranças vencidas', summary.followUps ? 'danger' : '')}</div>`;
  };
  ...
  const buildMarkup = (items) => {
    ...
    const canPrepareMessage = type === 'follow_up' || (item.responsible && !/não definido|sem responsável|andré|andre/i.test(item.responsible));
    return `<div class="pmh-decision-top"><div class="pmh-decision-main"><small>🤖 O QUE PRECISA DA SUA ATENÇÃO AGORA</small><div class="pmh-decision-kind tone-${esc(meta.tone)}">${esc(meta.icon)} ${esc(meta.label)}</div><h2>${esc(item.title)}</h2><p>${esc(reason)}</p><div class="pmh-decision-next"><small>PRÓXIMO MOVIMENTO</small><strong>${esc(movement)}</strong></div><div class="pmh-decision-meta"><span class="pmh-active-origin tone-${esc(item.originTone)}">${esc(item.origin)}</span><span>${esc(item.dependsOn || item.responsible || 'Sem responsável')}</span><time class="${esc(due.tone)}">${esc(due.label)}</time></div></div>
      <div class="pmh-decision-actions"><button type="button" class="primary" data-attention-open="${esc(item.id)}">Abrir direto na tarefa</button><button type="button" data-radar-context="${esc(item.id)}">${isDeferred(item) ? 'Editar dependência' : 'Adicionar contexto'}</button><button type="button" data-analyze-radar>Analisar contexto</button>${canPrepareMessage ? `<button type="button" data-copy-follow-up data-title="${esc(item.title)}" data-responsible="${esc(item.dependsOn || item.responsible)}" data-reason="${esc(item.blockerReason || reason)}">Preparar cobrança</button>` : ''}</div></div>
      ...
      <div class="pmh-decision-signals" aria-label="Leitura rápida do Radar">${signal(summary.executable, ' executáveis')}${signal(summary.dependencies, ' com dependência', summary.dependencies ? 'warning' : '')}${signal(summary.followUps, ' cobranças vencidas', summary.followUps ? 'danger' : '')}${signal(summary.late, ' atrasadas')}${signal(summary.approvals, ' aprovações executáveis')}</div>`;
  };

  const render = () => {
    const target = mount();
    if (!target || !snapshot || !radar()) return;
    const nextMarkup = buildMarkup(Array.isArray(snapshot.items) ? snapshot.items : []);
    if (nextMarkup === lastMarkup && target.innerHTML) return;
    lastMarkup = nextMarkup;
    target.innerHTML = nextMarkup;
  };

  const sync = async () => {
    if (syncing || !mount() || !radar()) return;
    syncing = true;
    try {
      snapshot = radar().getSnapshot() || await radar().collect();
      render();
    } catch {
      // A fila operacional já apresenta falhas de fonte.
    } finally {
      syncing = false;
    }
  };

  const findItem = (itemId) => snapshot?.items?.find((item) => item.id === itemId) || radar()?.getSnapshot()?.items?.find((item) => item.id === itemId) || null;
  const routeTriggerFor = (item) => {
    if (!item || item.action === 'demand') return null;
    return [...document.querySelectorAll(`[data-view="${attrValue(item.action)}"]`)].find((element) => !element.closest('[data-decision-cockpit], [data-attention-handoff]')) || null;
  };
  ...
  const highlightByTitle = (item) => {
    ...
    const target = [...document.querySelectorAll('article, button, section')]
      .filter((element) => !element.closest('[data-decision-cockpit], [data-attention-handoff]'))
      .find((element) => normalize(element.textContent).includes(targetTitle));
  ...
  const renderHandoff = () => {
    ...
    if (!handoff || !document.querySelector('#pmh-app')) return;
    ...
    card.innerHTML = `<header><span>${esc(meta.icon)} ${esc(meta.label)}</span><button type="button" data-attention-close aria-label="Fechar">×</button></header><strong>${esc(handoff.title)}</strong><p>${esc(handoff.reason)}</p><div><small>PRÓXIMO MOVIMENTO</small><b>${esc(handoff.nextAction)}</b></div><footer><button type="button" data-attention-context="${esc(handoff.itemId)}">Editar contexto</button><button type="button" class="primary" data-attention-reopen>Reabrir tarefa</button></footer>`;
  ...
  const tryOpenExact = (item, attempt = 0) => {
    ...
    if (target) {
      target.scrollIntoView({ behavior: 'smooth', block: 'center' });
      target.classList.add('pmh-attention-target');
      setTimeout(() => { target.click(); setTimeout(() => target.classList.remove('pmh-attention-target'), 2600); }, 180);
      return;
    }
    if (!selector && attempt >= 6 && highlightByTitle(item)) return;
    if (attempt >= 40) return void highlightByTitle(item);
    openTimer = setTimeout(() => tryOpenExact(item, attempt + 1), 100);
  };
  ...
  const openAttention = (itemId) => {
    ...
    saveHandoff({ itemId: item.id, sourceId: item.sourceId, action: item.action, title: item.title, origin: item.origin, type: entry.type, reason: entry.reason, nextAction: entry.nextAction, createdAt: Date.now() });
    renderHandoff();
    routeTriggerFor(item)?.click();
    tryOpenExact(item);
  };

  const copyFollowUp = async (button) => {
    const text = `Oi, ${button.dataset.responsible || ''}. Sobre “${button.dataset.title || ''}”: ${button.dataset.reason || ''} Consegue me confirmar o que falta e uma previsão para liberarmos essa demanda?`;
    try {
  ...
  window.addEventListener('pmh:view-rendered', (event) => {
    lastMarkup = '';
    if (event.detail?.view === 'inicio') sync();
    renderHandoff();
  });
  ...
  document.addEventListener('click', (event) => {
    ...
    const copy = event.target.closest('[data-copy-follow-up]');
    if (copy) return void copyFollowUp(copy);
    if (event.target.closest('[data-attention-close]')) return clearHandoff();
    const context = event.target.closest('[data-attention-context]');
    if (context) return void window.PMHRadarContext?.open(context.dataset.attentionContext);
    if (event.target.closest('[data-attention-reopen]') && handoff) openAttention(handoff.itemId);
  }, true);
})();
