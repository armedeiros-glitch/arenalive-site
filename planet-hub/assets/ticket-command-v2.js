(() => {
  ...
  const activeFollowedTickets = (tickets, followed = state.followed) => (tickets || []).filter((ticket) => isActiveTicket(ticket) && followed.has(ticketId(ticket)));
  ...
  const matchesBaseFilters = (ticket) => {
    ...
    const haystack = normalize([
      ticket.title, ticket.unit, ticket.requester, ticket.responsible, ticket.subject, ticket.department,
      ticket.sultsTicketId, context?.reason, context?.dependsOn, context?.nextAction, ...supportNames(ticket),
    ].join(' '));
  ...
  const followButton = (ticket) => {
    ...
    const busy = state.followingBusy.has(id);
    const followed = isFollowed(ticket);
    return `<button type="button" class="pmh-ticket-follow-button ${followed ? 'is-followed' : ''}" data-command-follow="${esc(id)}" aria-pressed="${followed ? 'true' : 'false'}" ${busy ? 'disabled' : ''}>${busy ? 'Salvando…' : followed ? 'Remover dos meus chamados' : 'Adicionar aos meus chamados'}</button>`;
  };
  ...
  const renderTicket = (ticket) => {
    ...
    return `<article class="pmh-ticket pmh-command-ticket ${deadline.tone === 'late' ? 'late' : ''} ${deferred ? 'has-operational-context' : ''}" data-ticket-id="${esc(id)}">
      <small>#${esc(id)}${isFollowed(ticket) ? ' · Em acompanhamento' : ''}</small>
  ...
  const renderGroup = (key, label, note, tickets) => `<section class="pmh-command-group group-${key}">
    ...
    <div class="pmh-command-list">${tickets.length ? sortTickets(key, tickets).map(renderTicket).join('') : '<div class="pmh-command-empty">Nenhum chamado neste grupo.</div>'}</div>
  </section>`;
  const renderDiscovery = (tickets) => `<section class="pmh-command-group group-progress" data-command-discovery-list>
    <header><div><span></span><div><h3>Todos os chamados ativos do SULTS</h3><p>Use esta lista para escolher quais chamados entram na sua fila do André OS.</p></div></div><b>${tickets.length}</b></header>
    <div class="pmh-command-list">${tickets.length ? sortTickets('progress', tickets).map(renderTicket).join('') : '<div class="pmh-command-empty">Nenhum chamado ativo encontrado com estes filtros.</div>'}</div>
  </section>`;
  const metricButton = (key, label, count, note, tone) => `<button type="button" class="pmh-command-metric ${tone} ${state.urgency === key ? 'active' : ''}" data-command-urgency="${key}"><small>${esc(label)}</small><strong>${count}</strong><span>${esc(note)}</span></button>`;
  ...
  const render = (mount, head) => {
    ...
    const followedActive = activeFollowedTickets(activeTickets);
    if (state.status && !ACTIVE_SITUATIONS.has(Number(state.status))) state.status = '';
    const base = followedActive.filter(matchesBaseFilters);
    ...
    const optionSource = state.discovery ? activeTickets : followedActive;
    ...
    const statusOptions = uniqueSorted(optionSource.map((ticket) => String(ticket.situation || ''))).map((value) => ({ value, label: STATUS[Number(value)] || `Status ${value}` }));

    if (head) head.innerHTML = `<div><small>CENTRAL DE DECISÃO</small><h2>Meus chamados</h2><p>${followedActive.length} chamado${followedActive.length === 1 ? '' : 's'} ativo${followedActive.length === 1 ? '' : 's'} adicionado${followedActive.length === 1 ? '' : 's'} por você.</p></div>`;
    ...
    mount.innerHTML = `
      ...
      <section class="pmh-command-filters">
        ...
        <select data-command-filter="status" aria-label="Filtrar por status"><option value="">Todos os status</option>${statusOptions.map((item) => `<option value="${esc(item.value)}" ${item.value === state.status ? 'selected' : ''}>${esc(item.label)}</option>`).join('')}</select>
      ...
      ${state.discovery ? renderDiscovery(discoveryTickets) : followedActive.length ? `<section class="pmh-command-groups">
      ...
      </section>` : '<div class="pmh-command-empty pmh-command-following-empty"><strong>Nenhum chamado adicionado ainda.</strong><span>Abra “Todos do SULTS” e adicione somente os chamados que você quer acompanhar aqui.</span></div>'}`;
  };

  const loadTickets = async (force = false) => {
    if (Array.isArray(state.tickets) && !force) return state.tickets;
    if (state.loading && !force) return state.loading;
    state.loading = fetch(API, { headers: { Accept: 'application/json' }, cache: 'no-store' }).then(async (response) => {
  ...
  const loadContexts = async (force = false) => {
    ...
    state.contextsLoading = fetch(CONTEXT_API, { headers: { Accept: 'application/json' }, cache: 'no-store' }).then(async (response) => {
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || `Falha HTTP ${response.status}`);
      state.contexts = new Map((Array.isArray(payload.data) ? payload.data : []).filter((item) => String(item.itemId || '').startsWith('ticket-')).map((item) => [String(item.itemId), item]));
      return state.contexts;
    }).catch(() => state.contexts).finally(() => { state.contextsLoading = null; });
    return state.contextsLoading;
  };
  const loadFollowing = async (force = false) => {
    if (state.followed.size && !force) return state.followed;
    if (state.followingLoading && !force) return state.followingLoading;
    state.followingLoading = fetch(FOLLOWING_API, { headers: { Accept: 'application/json' }, cache: 'no-store' }).then(async (response) => {
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || `Falha HTTP ${response.status}`);
      state.followed = new Set((Array.isArray(payload.data) ? payload.data : []).map((item) => String(item.id || '')).filter(Boolean));
      return state.followed;
    }).finally(() => { state.followingLoading = null; });
    return state.followingLoading;
  };
  const saveFollowing = async (id, follow) => {
    if (!id || state.followingBusy.has(id)) return;
    state.followingBusy.add(id);
    ...
    try {
      ...
      window.alert?.(`Não foi possível ${follow ? 'adicionar' : 'remover'} o chamado: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      state.followingBusy.delete(id);
      render(document.querySelector('.pmh-ticket-command'), document.querySelector('.pmh-section-head'));
    }
  };
  ...
  const contextsFromRadar = (snapshot) => {
    ...
    items.filter((item) => item.origin === 'SULTS' && String(item.id || '').startsWith('ticket-')).forEach((item) => {
      ...
      next.set(String(item.id), { itemId: String(item.id), state: item.operationalState || 'actionable', reason: item.blockerReason || '', dependsOn: item.dependsOn || '', nextAction: item.nextAction || '', followUpDate: item.followUpDate || '', updatedAt: item.contextUpdatedAt || '' });
    });
    state.contexts = next;
  };
  ...
  const transform = async () => {
    ...
    const head = kanban.previousElementSibling?.classList.contains('pmh-section-head') ? kanban.previousElementSibling : document.querySelector('.pmh-section-head');
    ...
    try {
      await Promise.all([loadTickets(), loadContexts(), loadFollowing()]);
      render(mount, head);
    } catch (error) {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (follow) {
      ...
      const id = String(follow.dataset.commandFollow || '');
      saveFollowing(id, !state.followed.has(id));
      return;
    }
  ...
  window.TicketCommandFollowing = Object.freeze({
    isActiveTicket,
    ticketId,
    activeFollowedTickets: (tickets, ids) => activeFollowedTickets(tickets, ids instanceof Set ? ids : new Set(ids || [])),
  });
