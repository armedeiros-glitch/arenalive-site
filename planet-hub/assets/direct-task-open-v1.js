(() => {
  ...
  const openDemand = async (handoff) => {
    const previewTitle = document.querySelector('[data-demand-preview] [name="title"]');
    if (previewTitle && normalizeText(previewTitle.value) === normalizeText(handoff?.title)) return true;
    const id = sourceIdOf(handoff);
